...
contains_debug_statements() {
	echo "$1" | grep -q -E 'dd\s*\(|console\.log\s*\('
}
	...
	if [[ -f "$filename" ]]; then
		if [[ "$filename" == *.blade.php || "$filename" == *.php || "$filename" == *.js ]]; then
			...
			while IFS= read -r line; do
				...
				if contains_debug_statements "$line"; then
					echo "File ${filename}:${line_number} Line \"$line\" contains debug statements (dd or console.log)"
...
if $need_throw_error; then
	echo "Find files with debug statements."
	exit 1
fi
